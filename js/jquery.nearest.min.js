...
(function ( e, t )
{
    ...
    {
        t || (t = "div");
        var s = e( r.container ), o = s.offset() || {
                left: 0,
                top: 0
            }, u = [o.left + s.width(), o.top + s.height()], a = {x: 0, y: 1, w: 0, h: 1}, f, l;
        for( f in a )
        {
            a.hasOwnProperty( f ) && (l = n.exec( r[f] ), l && (r[f] = u[a[f]] * l[1] / 100));
        }
        ...
        !r.includeSelf && i && (c = c.not( i )), E < 0 && (E = 0), c[S ? "each2" : "each"]( function ( t, n )
        {
            ...
            if( d && v || !d && !v && L && A || d && A || v && L )
            {
                O = L ? 0 : c - N, M = A ? 0 : C - k, _ = L || A ? T( O, M ) : Math.sqrt( O * O + M * M ), D = p ? _ >= m - E : _ <= m + E, D && (m = p ? T( m, _ ) : x( m, _ ), h.push( {
                    node: this,
                    dist: _
                } ))
            }
        ...
        if( N )
        {
            ...
            for( A = 0; A < N; A++ )
            {
                O = h[A], O.dist >= k && O.dist <= L && C.push( O.node )
            }
        }
        return C
    }
    ...
    e.each( ["nearest", "furthest", "touching"], function ( n, i )
    {
        ...
        {
            var i;
            if( t && e.isPlainObject( t ) )
            {
                return i = e.extend( {}, s, t, n || {} ), this.pushStack( r( this, i ) );
            }
